...
function makeDedupKey(gateway: string, gatewayMessageId: string): string {
...
export const sendOutbound = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!session) {
      try {
        const directSession = await ctx.db.get(args.sessionId as any);
        if (directSession) {
...
export const getOrCreateSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      return { sessionId: existing._id as string, created: false };
    }
    ...
    return { sessionId: id as string, created: true };
  },
});
...
export const listSessions = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Apply filters
    if (args.activeOnly) {
      sessions = sessions.filter((s: any) => s.active);
    }

    // Sort by lastActivityAt descending
    sessions.sort((a: any, b: any) => b.lastActivityAt - a.lastActivityAt);

    return sessions.slice(0, limit).map((s: any) => ({
...
export const getMessages = query({
  ...
  handler: async (ctx, args) => {
    ...
    let messagesQuery = ctx.db
      .query("messages")
      .withIndex("by_session_timestamp", (q: any) => {
...
export const listGateways = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.activeOnly) {
      gateways = gateways.filter((g: any) => g.active);
    }
...
export const pruneSessions = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const sessions = await ctx.db
      .query("sessions")
      .withIndex("by_active_lastActivity", (q) =>
        q.eq("active", false).lt("lastActivityAt", cutoff)
      )
      .collect();
    ...
    for (const session of sessions) {
      if (shouldDeleteMessages) {
        ...
        const messages = await ctx.db
          .query("messages")
          .withIndex("by_session_timestamp", (q: any) =>
            q.eq("sessionId", session._id as string)
          )
          .collect();
